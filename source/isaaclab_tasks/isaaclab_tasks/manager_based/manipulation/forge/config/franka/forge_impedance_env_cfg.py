...
# Use Nucleus server for assets (requires proxy env vars in China)
FORGE_ASSET_DIR = f"{ISAACLAB_NUCLEUS_DIR}/Factory"

##
# Franka-specific actuator config for impedance control using Factory robot model.
# Uses franka_mimic.usd which has panda_fingertip_centered body for EE tracking.
##

FORGE_FRANKA_CFG = ArticulationCfg(
    prim_path="{ENV_REGEX_NS}/Robot",
    spawn=sim_utils.UsdFileCfg(
        usd_path=f"{FORGE_ASSET_DIR}/franka_mimic.usd",
...
class FrankaPegInsertSceneCfg(ForgeSceneCfg):
    ...
    # Fixed asset: Hole (use ArticulationCfg with empty actuators, matching direct version)
    # Note: joint_pos={} and joint_vel={} override defaults since these have no joints
    fixed_asset = ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            pos=(0.6, 0.0, 0.05), rot=(1.0, 0.0, 0.0, 0.0),
            joint_pos={}, joint_vel={}
        ),
        actuators={},
    )

    # Held asset: Peg (use ArticulationCfg with empty actuators, matching direct version)
    # Note: joint_pos={} and joint_vel={} override defaults since these have no joints
...
class FrankaGearMeshSceneCfg(ForgeSceneCfg):
    ...
    # Fixed asset: Gear base (use ArticulationCfg with empty actuators)
    fixed_asset = ArticulationCfg(
        prim_path="{ENV_REGEX_NS}/FixedAsset",
        spawn=sim_utils.UsdFileCfg(
    ...
    # Held asset: Medium gear (use ArticulationCfg with empty actuators)
    held_asset = ArticulationCfg(
        prim_path="{ENV_REGEX_NS}/HeldAsset",
        spawn=sim_utils.UsdFileCfg(
...
class FrankaNutThreadSceneCfg(ForgeSceneCfg):
    ...
    # Fixed asset: Bolt (use ArticulationCfg with empty actuators)
    fixed_asset = ArticulationCfg(
        prim_path="{ENV_REGEX_NS}/FixedAsset",
        spawn=sim_utils.UsdFileCfg(
    ...
    # Held asset: Nut (use ArticulationCfg with empty actuators)
    held_asset = ArticulationCfg(
        prim_path="{ENV_REGEX_NS}/HeldAsset",
        spawn=sim_utils.UsdFileCfg(
