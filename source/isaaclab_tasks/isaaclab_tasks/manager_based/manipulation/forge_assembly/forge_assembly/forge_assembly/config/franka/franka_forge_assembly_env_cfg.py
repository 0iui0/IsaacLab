...
class FrankaForgeAssemblyEnvCfg(ForgeAssemblyEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.robot = ArticulationCfg(
            ...
            actuators={
                "panda_arm1": ImplicitActuatorCfg(
                    ...
                    stiffness=0.0,  # Pure torque control (match direct forge)
                    damping=0.0,
                    friction=0.0,
                    armature=0.0,
                ...
                "panda_hand": ImplicitActuatorCfg(
                    ...
                    effort_limit_sim=40.0,  # Match direct forge
                    velocity_limit_sim=0.04,  # Match direct forge
                    stiffness=7500.0,  # Match direct forge
                    damping=173.0,  # Match direct forge
                    friction=0.1,
                ),
            },
        ...
        self.actions.arm_action = ForgeAssemblyActionCfg(
            ...
            hole_name="hole",
            peg_name="peg",
            # Match direct forge exactly: pos_action_bounds=0.05, threshold=0.02
            pos_action_bounds=[0.05, 0.05, 0.05],
            # Match direct forge exactly: rot_action_bounds=1.0 (full rotation range)
            rot_action_bounds=[1.0, 1.0, 1.0],
            # ForgeCtrlCfg overrides: [565, 565, 565, 28, 28, 28]
            default_task_prop_gains=[565.0, 565.0, 565.0, 28.0, 28.0, 28.0],
            default_dead_zone=[5.0, 5.0, 5.0, 1.0, 1.0, 1.0],
            # Match direct forge: threshold swapped vs bounds (threshold=0.02, bounds=0.05)
            ...
            kp_null=10.0,
            # Match direct forge: kd_null = 2*sqrt(kp) = 2*sqrt(10) = 6.3246
            kd_null=6.3246,
            # Match direct forge CtrlCfg.default_dof_pos_tensor (nullspace target)
